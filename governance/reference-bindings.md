...
|---|---|---|---|---|
| `mimir.alertmanager.v1` | Grafana Mimir + Prometheus Alertmanager (Grafana Alloy translation tier) | §3.X.TEL.11 / §3.82.11; Gateway contract §3.X.TEL.7–14 / §3.82.7–14 | the binding catalog (forthcoming) | ACTIVE |

At v2.1.2, `mimir.alertmanager.v1` is the single registered binding, per the registry clause of §3.X.TEL.11 (V10):
...
- New bindings MAY be registered through subsequent AIMS RFCs, per the §3.X.TEL.11 registry clause quoted above and the RFC process described in `CONTRIBUTING.md` (30-day default comment period; maintainer decision recorded in the project's governance records).
